...
- Linear mixed-effects model (for repeated measurements per subject): 
  - `dev_ermod_lme()`, `dev_ermod_lme_exp_sel()`
  - `dev_ermod_cqt()` for concentration-QTc (C-QT) analysis; see the
    [C-QT analysis article](https://genentech.github.io/BayesERtools/articles/cqt_analysis.html)
...
For mixed-effects models, `sim_er()`, `sim_er_new_exp()`, and
`sim_er_curve()` can simulate population-level predictions (default),
new subjects with between-subject variability, or existing subjects with
their posterior random effects, controlled by the `re_type` argument.
